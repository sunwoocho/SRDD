...
from data_ost import ImageFolder
...
def main(args):
    ...
    print('argument nclass: ', args.nclass)
    ...
    train_steps = 0
    log_steps = 0
    running_loss, running_loss_pos, running_loss_neg, running_loss_sr = 0, 0, 0, 0
    start_time = time()
    ...
    for epoch in range(args.epochs):
        ...
        for x, ry, y in loader:
            ...
            lr = F.interpolate(x, scale_factor=4, mode='bicubic', align_corners=True)
            ilr = F.interpolate(lr, size=(height, width), mode='bicubic', align_corners=True)
            criterion = torch.nn.MSELoss()
            sr_loss = criterion(x, ilr)
            ...
            if args.condense:
                ...
                all_loss = loss + pos_match_loss + neg_match_loss - sr_loss
            else:
                all_loss = loss
            ...
            if train_steps % args.log_every == 0:
                ...
                avg_loss_sr = torch.tensor(running_loss_sr / log_steps, device=device)
                dist.all_reduce(avg_loss, op=dist.ReduceOp.SUM)
                dist.all_reduce(avg_loss_pos, op=dist.ReduceOp.SUM)
                dist.all_reduce(avg_loss_neg, op=dist.ReduceOp.SUM)
                dist.all_reduce(avg_loss_sr, op=dist.ReduceOp.SUM)
                avg_loss = avg_loss.item() / dist.get_world_size()
                avg_loss_pos = avg_loss_pos.item() / dist.get_world_size()
                avg_loss_neg = avg_loss_neg.item() / dist.get_world_size()
                avg_loss_sr = avg_loss_sr.item() / dist.get_world_size()
                logger.info(f"(step={train_steps:07d}) Train Loss: {avg_loss:.4f} {avg_loss_pos:.4f} {avg_loss_neg:.4f} {avg_loss_sr:.4f}, Train Steps/Sec: {steps_per_sec:.2f}")
            ...
            if train_steps % args.ckpt_every == 0 and train_steps > 0:
                if rank == 0:
                    ...
                    checkpoint_path = f"{checkpoint_dir}/last_ckpt.pt"
                    torch.save(checkpoint, checkpoint_path)
                    logger.info(f"Saved checkpoint to {checkpoint_path} with step {train_steps:07d}")
                dist.barrier()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nclass", type=int, default=1, help='the class number for distillation training')
